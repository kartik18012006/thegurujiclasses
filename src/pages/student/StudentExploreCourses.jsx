...
const StudentExploreCourses = () => {
  ...
  const fetchEnrolledCourseIds = async () => {
    ...
    try {
      ...
      const enrolledIds = [];
      const checkPromises = coursesSnapshot.docs.map(async (courseDoc) => {
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const teacherNameMatch = course.teacherName?.toLowerCase().includes(searchLower) || false;

    return titleMatch || instructorMatch || teacherNameMatch;
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          ...
          <p className="text-xl text-gray-600">Browse all available courses and enroll to start learning</p>
        </div>
        ...
        <div className="mb-8">
          <div className="relative max-w-2xl">
            ...
            <input
              ...
              className="block w-full pl-12 pr-12 py-4 bg-white/80 backdrop-blur-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 placeholder-gray-500 shadow-lg transition-all"
            />
            {searchText && (
              <button
        ...
        {courses.length === 0 ? (
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-gray-200 p-16 text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-3">No courses available</h2>
            <p className="text-gray-600 text-lg">There are no courses available at the moment.</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-gray-200 p-16 text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-3">No courses found</h2>
            <p className="text-gray-600 text-lg mb-8">No courses found for your search "{searchText}"</p>
            <button
              onClick={() => setSearchText('')}
          ...
          <>
            ...
            {searchText && (
              <div className="mb-6 text-lg text-gray-700 font-medium">
                Found {filteredCourses.length} {filteredCourses.length === 1 ? 'course' : 'courses'} matching "{searchText}"
              </div>
            )}
